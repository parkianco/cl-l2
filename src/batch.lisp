...
(defun compress-batch (batch)
  ...
  (let ((buffer (make-array 4096 :element-type '(unsigned-byte 8)
    ...
    (with-output-to-string (s)
      (let ((stream (flexi-streams:make-flexi-stream s)))
        (serialize-batch batch stream)))
    ;; For now, just return serialized form
    ;; Real implementation would apply compression
    buffer))
